...
| §5.48 Multiplayer — `principal.*` / `workspace.invite.*` / `invite.redeem` / `workspace.members.*` (principals, invite links, the `/invite` device-flow join, membership, the collaborator allowlists) | [methods/multiplayer.md](./methods/multiplayer.md) |
