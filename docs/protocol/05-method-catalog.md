...
| sandbox | 6 | cow.discard, cow.merge, image.check, options, profiles.list, profiles.update — `cow.discard` / `cow.merge` are the CoW agent-sandbox controls (§5.5a); the `sandbox.profiles.*` / `sandbox.options` / `sandbox.image.check` execution-environment profile surface is daemon-global (no `workspaceId`; §5.5b; v10.3) |
