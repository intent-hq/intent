...
- **Redirection (checkbox → task).** `task.updateStatus` and `task.update` on a linked
  ...
  **parent**, never the task note. **The caller-aware terminal guard** of
  `task.updateNoteStatus` applies to both redirects: a word that would move a `complete`
  / `cancelled` task, written by the task's OWN linked agent, is a full no-op — the task,
  the parent line and its `rev` are untouched and no `task:*` / `note:updated` fires.
  `task.update` resolves the guard **before** its parent write, so the line projects the
  status the task actually keeps: a same-text write persists nothing, a real `text` edit
  still lands in ONE parent write carrying the terminal marker, and the result's `status`
  echoes the marker the line carries (not the refused word). **Dangling links keep the raw checkbox write:** a
