...
>
> **Total exit contract (`exited` always carries `exitCode`, new in intentd, v9.66).** `exited`
...
> (§6.5) — and a `command` row (new in intentd, v9.66) hydrates as `exited` + `exitCode: -1` +
...
> Semantics:
>
> - **Set by every successful start/restart**, in both modes (new in intentd, v9.66 —
