...
>   events (§6.5).
> - `github.cancelAuth` aborts a pending flow — **scoped to the flow the caller started** when it
>   passes the `flowId` its `github.connect` returned (a stale id is a no-op, so one client cannot
>   cancel another's flow); `github.revoke` deletes the **stored** token (env / `gh` fallbacks are
...
| github.connect | — | { ok: true, userCode, verificationUri, expiresIn, interval, flowId } — starts the OAuth **device flow** (or returns the SAME codes while one is pending — idempotent). `flowId` is an **opaque string** identifying the started flow, unique per started flow within the daemon process; a concurrent connect that adopts the resident live flow returns the SAME `flowId` (it is not a device code and never sensitive — administrator-only surface). Callers hand it back to `github.cancelAuth` to cancel only their own flow. The daemon polls GitHub in the background; terminal transitions arrive as `github:auth-changed` events (§6.5). A missing/empty `sourceControl.github.oauthClientId` or an unreachable login host → `-32603` |
| github.cancelAuth | flowId? | { ok: true, cancelled } — aborts a pending device flow (`cancelled: true` iff one was pending; idempotent no-op otherwise). **Flow-scoped when `flowId` is present**: the pending flow is aborted only if its id equals the `flowId` a `github.connect` returned; a stale or unknown id ⇒ `{ ok: true, cancelled: false }` and nothing else is touched — the pending flow keeps polling and `github.authStatus` still reports it — so a client can never cancel a flow another caller started. An **omitted** `flowId` keeps the unscoped behaviour (aborts whatever flow is pending). A non-string `flowId` → `-32602`; "nothing pending" is never an error |
...
interface GithubPullRequest {
  ...
  deletions: number;
  changedFiles: number;
  owner?: string;          // repository owner — present on `github.pulls.search` items (v10.1); names the hit's own repo
  repo?: string;           // repository name — present on `github.pulls.search` items (v10.1)
}
...
interface GithubIssue {
  ...
  owner?: string;          // repository owner (echoed for convenience; on a multi-repo `github.issues.search` names the hit's own repo, v10.1)
...
// ← response — the user enters userCode at verificationUri; flowId is what THIS caller
//   would pass to github.cancelAuth { flowId } to back out of only its own flow
