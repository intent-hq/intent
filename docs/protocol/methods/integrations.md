...
interface GithubPullRequest {
  ...
  deletions: number;
  changedFiles: number;
  owner?: string;          // repository owner — present on `github.pulls.search` items (v9.14); names the hit's own repo
  repo?: string;           // repository name — present on `github.pulls.search` items (v9.14)
}
...
interface GithubIssue {
  ...
  owner?: string;          // repository owner (echoed for convenience; on a multi-repo `github.issues.search` names the hit's own repo, v9.14)
