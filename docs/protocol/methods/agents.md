...
1. **Session persistence** — the pending request is persisted on the caller's session as
   ...
   **user-origin** delivery — `agent.sendMessage` (the FE/router front door),
   ...
   entry (the same origin taxonomy as the §5.5 pending-questions answer intake). For **child**
...
below it) and clear it when there is none, then recompute displayStatus and kick the queue drain
(a best-effort nudge for entries parked by other gates — these paths start no turn of their own).
