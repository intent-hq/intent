...
the pending-questions derivation, and queue persistence/rehydration are all untouched by any mode.
...
- **Pending questions.** A pending structured-question set (below) does NOT gate the flush
  ...
  any origin drain and batch under the ordinary mode rules while the marker is set. The
...
- **Archived workspace.** The §5.1 archived gate is the one origin-aware drain gate
  ...
  exemption at all — the resurrection signal is a user-origin entry, which `systemOnly` by
  definition excludes — so the qualifying user-origin entry drains solo, exactly like the
...
the attention-request retire (§5.5 above), the `systemOnly` flush exclusion, and the archived-
workspace exemption.
