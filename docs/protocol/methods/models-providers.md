...
- `forceRefresh: true` skips the cache read, awaits a fresh probe, and stores the result on success. On failure it returns the **last-good** list labeled `stale: true` plus a `warning` — stale data is never served silently. It is the FE model picker's refresh button — the only client-driven way to re-probe a still-fresh cached provider on demand (aged entries re-probe on their own, per the next bullet).
- **Non-forced reads** serve a cached entry while it is fresh; a probe runs on a **true cache miss** — first use, or a version-key mismatch (e.g. after an adapter pin bump) — or on an **aged entry** (at or past the 24-hour threshold), so newly released provider models surface within a day without manual refresh. Probes are awaited inline (no stale-while-revalidate) with the same last-good + `warning` fallback on failure — a failed re-probe of an aged entry keeps serving the last-good list labeled `stale: true` + `warning`, so an aged entry never degrades below its last-good state.
- **Probe guards.** Concurrent probes for the same `(provider, version key)` are single-flighted (one spawn, shared result), and a failed probe is negatively cached for **60 seconds**: non-forced reads within the window short-circuit to the failure fallback without re-probing — a true miss serves the failed probe's degradation (static), an aged entry keeps being served as the stale-labeled last-good list; `forceRefresh` bypasses the negative entry (and the cache read) but is still single-flighted. A successful probe clears the negative entry; empty successes are served but never cached, so they never masquerade as a last-good list.
