...
| workspace.import.commit *(v6.9)* | importId (req) | { workspace, importedRows, interruptedAgents, rehydrated } — reassembles the staged chunks → verifies the SHA-256 against `begin`'s `archiveSha256` → unzips (zip-slip-safe extract) → checks the embedded manifest equals the `begin` manifest → applies row transforms (path rewrite against the target `workspaces_root`, agent sessions forced to stopped with `interrupted_agent` rows for in-flight agents, legacy compound model identities normalized, drafts dropped; `event` rows are excluded by export and rejected if present) → git materialization → attachment materialization (each `attachments/<attachmentId>` archive entry lands at the `stored_path` its registry row records, resolved against the materialized checkout with the within-workspace containment guard — an escaping `stored_path` fails the commit — dropping the ignore-all `.gitignore` marker in the store dir; a registry row with no archive entry is the deleted-is-deleted state and imports as a row without a file; a later failure unwinds every placed file) → asset placement (failure rejects and unwinds staged files) → single-transaction row insert → boot-style rehydration (hooks, event subscriptions, PR monitors). **Atomic:** nothing is visible in `workspace.list` until commit succeeds |
...
**Transfer compatibility and validation.**

Patch compatibility applies to **supported data**, not arbitrary future schemas.
Harmless additive manifest metadata is tolerated. Import rejects unknown tables
(including excluded event history), unsupported or duplicate archive entries and
columns absent from the receiving database. Row-scope checks, required values,
foreign keys, checksums and ID collisions remain enforced; failed row inserts roll
back the whole transaction rather than discard incompatible data. Asset placement
also fails before the row commit when the receiver cannot store the files;
pre-existing asset directories are not overwritten. Future changes
that cannot preserve this contract need a transfer-format change or an explicit
adapter, not a wider version gate.

Format-v1 archives from before attachment support may omit `attachments` (treated
as an empty list). Attachment files must have matching registry rows with valid
within-workspace stored paths; orphan files fail the commit. Missing attachment
files retain the existing deleted-is-deleted behavior: `exists` and `sizeBytes`
are plan-time metadata, and an exported file can disappear between plan and
archive creation. The archive checksum validates the actual bytes transferred.

**Legacy receivers still enforce their old exact-version gate.** Updating only the
source cannot enable cross-patch imports into an older receiver. These changes do
not migrate or downgrade either daemon's database; incompatible future table or
column changes are rejected safely even within a major/minor pair.
