...
**Merged `pullRequests` on the list emit paths (new in intentd).** On `workspace.list` and
the lite `workspace.subscribe` seq-0 snapshot (§6.9) — the two list surfaces — the emitted
`pullRequests` is a **merged pool**, not a bare read of the stored column: the daemon folds
in the PRs persisted on the workspace's registered git roots
...
excluded — the same visibility rule as `prMonitor.list`). **Dedup is by PR `url`** — one
entry per URL; when the same PR appears in multiple sources, the richest / most recently
updated entry wins (workspace > git-root > monitor-derived). Entries can therefore be
...
monitored PRs without opening the workspace. Everything else is unchanged: the stored
`workspace.pull_requests` column keeps its workspace-repo semantics (PR discovery/refresh
writes it as before, and the explicit-null clear below still targets only the stored
value), `workspace.get` and the write-path responses carry the unmerged workspace-level
list, and the `pr:*` event payloads (§6.5) are untouched.
