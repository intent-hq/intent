...
> key is **omitted** (never `null` or `0`) — the same absence as
> `requirements.threads.unresolved` — rather than counting every thread as
> unresolved; that fallback is logged at warn level with the underlying GraphQL
