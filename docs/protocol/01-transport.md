...
- **Credit-exhausted close.** A stream whose credit stays at zero for the idle timeout **while loopback output is waiting behind it** is torn down with the final `CLOSE` (the daemon logs a warning naming `credit_exhausted_for_ms`). A zero-credit stream with nothing waiting is *not* closed by this rule — it lives or dies by the ordinary idle timeout like any other stream — and a client `CREDIT` cancels the pending deadline.
- **Compatibility.** The window is a fixed daemon default, not negotiated at `OPEN`, and `OPEN_OK` is unchanged — the daemon does not advertise credit support. Pre-credit clients (any build that never sends `CREDIT`) therefore keep working unchanged for the first 1 MiB per stream and then stall only that stream. In the other direction a daemon that predates `CREDIT` treats it as an unknown opcode and closes the whole connection with `1002`; `CREDIT` rides protocol version 10.3 without a bump (see [versioning](./versioning.md)), so the daemon side must ship before a credit-sending client is pointed at it.
