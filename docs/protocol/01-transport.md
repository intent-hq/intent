...
| `0x07` | `CREDIT` | credit `u32` BE, non-zero | client → daemon |

Malformed frames — shorter than the 5-byte header, an unknown opcode, an `OPEN` payload that is not exactly 2 bytes, a `CREDIT` payload that is not exactly 4 bytes or grants `0` bytes, a payload on a payload-less opcode (`OPEN_OK` / `EOF` / `CLOSE`), or a non-UTF-8 `OPEN_ERR` message — and the daemon-only opcodes (`OPEN_OK` / `OPEN_ERR`) arriving from the client end the whole **connection** with a `1002 Protocol Error` close naming the violation. `CREDIT` is client-only in the same sense: a client treats a daemon→client `CREDIT` as a protocol violation.
...
| Daemon → client credit window (per stream) | 1 MiB initial, replenished by client `CREDIT` | at zero credit only that stream's loopback read pauses; a stream starved of credit while loopback output is waiting is torn down after the idle timeout (final `CLOSE`) |
...
Inbound byte permits remain held through a blocked TCP write and are released on completion, cancellation or teardown. Full stream queues or exhausted shared bytes close only the offending stream without parking the shared reader. Daemon-side TCP reads produce at most 16 KiB per frame; the outbound queue holds 64 frames. Generation identity prevents stale output/cleanup from crossing a reused stream ID. Desktop diagnostics report configured limits, active/pending counts, port, generation and last admission failure/wait; no application payload is recorded.

**Per-stream flow control (daemon → client `DATA`)** ([intent-hq/intent#5482](https://github.com/intent-hq/intent/issues/5482); [intent-hq/intentd#2022](https://github.com/intent-hq/intentd/pull/2022)). Daemon→client `DATA` is credit-bounded per stream so that one client-side consumer that stops draining cannot park the connection's single WebSocket write for every sibling stream and the heartbeat:

- **Initial window.** Every stream starts with **1 MiB** of credit at `OPEN_OK`, granted without any client action. Each daemon→client `DATA` payload consumes its length; the daemon reads its loopback socket in at most `min(16 KiB, credit)` chunks and never sends a `DATA` byte it has no credit for.
- **`CREDIT` replenishes.** The client sends `CREDIT` with the number of additional payload bytes the daemon may send on that stream — the bytes it has actually flushed to its local socket, not bytes merely received. The payload is exactly 4 bytes and must be non-zero (`1002` otherwise); grants accumulate and **saturate at `u32::MAX`**. A `CREDIT` for an unknown stream id is ignored, like any other frame racing a teardown. The desktop client coalesces grants (one `CREDIT` per 64 KiB flushed, or per flush of a backlog, whichever comes first).
- **Zero credit pauses only that stream's loopback read.** The TCP socket stays open, the stream's client→daemon direction keeps relaying, and sibling streams and the heartbeat keep flowing. Because the loopback read is what observes the peer's end of stream, a peer `EOF` (or read error) reaches the client only once the stream holds credit again — a client should keep granting flushed bytes to the end of a reply.
- **Credit-exhausted close.** A stream whose credit stays at zero for the idle timeout **while loopback payload is waiting behind it** (the daemon observes this with a non-consuming peek) is torn down with the final `CLOSE` (the daemon logs a warning naming `credit_exhausted_for_ms`). A zero-credit stream with nothing waiting is *not* closed by this rule — it lives or dies by the ordinary idle timeout like any other stream — and a client `CREDIT` cancels the pending deadline. A peer half-close seen while the window is empty is not waiting output: it is remembered and forwarded as `EOF` by the first credited read, so a peer that sends exactly the window and half-closes while the client's upload continues keeps its stream.
- **Compatibility.** `CREDIT` ships with protocol version **10.4** (additive minor bump; see [versioning](./versioning.md)). The window is a fixed daemon default, not negotiated at `OPEN`, and `OPEN_OK` is unchanged — the version is the capability signal: a client sends `CREDIT` **only when the daemon's `client.hello` `protocolVersion` is ≥ 10.4** (numeric major.minor comparison — 10.10 and any major > 10 qualify; a missing or unparsable version does not) and otherwise behaves exactly as a pre-credit client, tracking flushed bytes but sending nothing. Pre-credit clients (any build that never sends `CREDIT`) keep working unchanged against a 10.4 daemon for the first 1 MiB per stream and then stall only that stream. A daemon that predates 10.4 treats `CREDIT` as an unknown opcode and closes the whole connection with `1002`, which the version gate prevents; the daemon side still ships first.
- **Scope of the guarantee.** It holds for a credit-aware client that keeps reading the WebSocket and merely stops flushing one local socket. The initial window is fixed, not negotiated, so a peer that stops reading the WebSocket altogether can still block the connection write before its window is spent; a bounded connection write for that case is a separate follow-up.
