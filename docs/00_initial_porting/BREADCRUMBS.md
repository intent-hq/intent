...
- `packages/intentd` @ `278f815` (after Milestone 4 — Cycle B: GitHub / PR — `intent-sourcecontrol` + `pr.*` + PR↔workspace linking)

## Implemented surface so far

- **JSON-RPC methods (55 request methods + 1 server-initiated notification):** full CRUD across four domains plus the event and PR surface over UDS, backed by SQLite —
