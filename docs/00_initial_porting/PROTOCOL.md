...
> **`Linked-Note-Id:` deferred.** The reference's MCP `agentCommit` writes **`Agent-Id:` only**; `Linked-Note-Id:` is supplied solely by the auto-commit-on-task-completion path (`auto-commit.service.ts`), which is **not yet ported**. The `linked_note_id` parameter is plumbed end-to-end but currently written as `None`, so the read side is fed the moment that path lands.
