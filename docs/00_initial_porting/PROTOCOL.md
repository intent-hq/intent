...
| git / terminal / test / build | git:, terminal:command, test:, build:* | Mostly reserved-but-unused in the reference impl. `git:commit` is emitted by `git.commit` / `git.agentCommit` (§5.6) with `data { workspaceId, operation: "commit", commit, message, files }` (matches the reserved FE `GitOperationEvent` shape); the successful commit path also emits `changes:git-status` so subscribers can refresh without a follow-up `git.status`. |
