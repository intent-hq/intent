...
- The daemon emits `skills:changed` (§6.5) when the discovered skill set changes for a workspace (check-on-read pattern: the `skill.list` handler compares the newly-discovered set against the cached fingerprint and publishes the event if they differ).
...
| skills (new in intentd) | skills:changed | Emitted when the discovered skill set changes for a workspace (check-on-read pattern: the `skill.list` handler compares the newly-discovered set against the cached fingerprint and publishes the event if they differ). data = { workspaceId }. Clients should re-fetch via `skill.list` (§5.34) to refresh the skill roster. New in intentd — not part of the ported reference taxonomy. |
