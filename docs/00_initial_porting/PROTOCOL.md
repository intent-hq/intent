...
**Commit attribution trailers (write side) — `Agent-Id:`.** The agent commit path now **writes** the `Agent-Id:` trailer that the Code Changes Review read side already parses (`git.commits` and the `file-tracking.*`/change-metrics reads, §5.18–§5.20, via `intent-git`'s `parse_trailers`) — closing the prior read-only asymmetry. The body is built by `build_commit_message` (a port of the reference `agent-commit.service.ts::buildCommitMessage`): the message is trimmed, runs of 3+ newlines collapse to a single blank line, and `Agent-Id: <id>` is appended after one blank line (round-trips with the parser). The wire shapes above are **unchanged** — `git.agentCommit` is still `message (req), files?, userRequested? → { ok, hash, files, fileCount }`; the trailer is written only when an agent context is present.

> **Option B — attribution from the MCP caller context.** Matching the reference `ws-git-api.ts`, the agent id is **not** a wire parameter: it is sourced from the **MCP caller context** (the agent that is committing). This is exposed via the agent-facing named MCP tool **`git_commit_workspace-mcp`** (declared in intentd's tool catalog, completing the previously reserved-but-unimplemented entry in the git denylist; see the agent→BE MCP callback, IMPLEMENTATION_SPEC.md §6.8). The tool takes `message (req), files?, userRequested?`, threads the caller agent id into `git_agent_commit`, honors the `userRequested` auto-commit gate, and errors with `"No agent context available. This tool must be called by an agent."` when no agent context is present. The wire-facing `git.agentCommit` (called without agent context) writes no trailer.

> **`Linked-Note-Id:` shipped (auto-commit-on-turn-end producer).** Matching the reference's `auto-commit.service.ts`, intentd now auto-commits an idle task-linked agent's working-tree changes when the agent finishes a normal turn — a daemon-side `agent:idle` subscriber reuses the existing `git_agent_commit` write path with the idle agent's own id as `agent_id` and the agent's persisted `task_note_id` as `linked_note_id`, so the body carries both `Agent-Id:` and `Linked-Note-Id:` trailers (round-trips via `parse_trailers`, feeding `git.commits` + `file-tracking.*`/change-metrics). The MCP `git_commit_workspace-mcp` path is unchanged (it writes `Agent-Id` only — the reference does the same; `Linked-Note-Id` comes exclusively from the auto-commit path). Silent skips: `git.autoCommit` disabled, per-agent `skip_auto_commit`, clean working tree, or non-normal `finishReason`. Deferred from this slice (documented follow-ups): AI-generated commit messages (`generateCommitMessage`) — uses the faithful fallback chain `taskTitle → meaningful agent name → "Agent changes"` (trimmed, newlines→spaces, ≤72 chars); pre-commit-hook-failure → wake-agent retry loop (N/A under libgit2); UI status history / domain events (`autoCommitHistory`, `gitAutoCommitStarted/Succeeded`, `backgroundGitOpsService`); per-agent file-tracking attribution filter (commit still takes all changed paths).
