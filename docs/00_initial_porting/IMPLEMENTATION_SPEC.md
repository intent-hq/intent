...
The **Integrations & Ops** domain (§19) follows the same pattern: `services::token_usage` (the daemon-internal periodic usage scanner + the `workspace.getTokenUsage` read), `services::session_stats` (per-session credit/message/tool stats via `auggie session stats --json` + the `agent.getSessionStats` read), `services::setup_scripts` (worktree setup-script persistence, project-type detection, and AI-assisted generation), and `services::repo_config` (per-repository configuration file persistence — `.intent/config.json` at the repository root — via the `repoConfig.*` namespace). They depend only on `intent-store` (the durable `tokenUsage`/`setupScript` workspace fields + the new `token_usage` table, §9.13), `intent-context`/`intent-providers` (for `auggie`/AI-assisted generation), `intent-git` (for repository root resolution), and the event bus. Usage/credit **scanning is internal** — only the reads + change events cross the wire (§19.1–19.2).
...
| intent-services | note/task/comment/workspace/agent/git/pr/script/file/event/draft logic (incl. the `services::drafts` module, §9.10) plus the Agent-Ecosystem modules `services::rules`, `services::specialists`, `services::mcp_servers`, `services::memories` (§18) and the Integrations & Ops modules `services::token_usage`, `services::session_stats`, `services::setup_scripts`, `services::repo_config` (§19) | core, store, git, sourcecontrol, acp, context, providers, pty, search |
...
> **Decision (2026-06-24): auggie has no codebase-retrieval CLI — ripgrep is the backing (Milestone 10, Option A).** Verified against canonical `augmentcode/intent` @ `6d97184` and auggie 0.29.0: the original `codebase:search` IPC handler was a **never-implemented stub returning `{ results: [] }` with no caller**, and `executeAuggieCommand` was only ever used for `model list` / `--version` / `session stats` — **never retrieval**. auggie exposes **no structured codebase-retrieval subcommand**. Accordingly `AuggieContextEngine::retrieve()` returns `Unavailable` **instantly** (no doomed spawn), and `search.codebase` stays **ripgrep/symbol-backed** (§14.2) — already exceeding the original's empty placeholder. The `ContextEngine` trait, binary discovery, and the `doctor` availability check are **retained as forward-looking infra** so a real retrieval engine can be slotted in later, but wiring auggie for retrieval is **not a gap** — there is genuinely nothing to port. A deliberate, faithful-port decision.
...
-- Long-term agent memory (§9.12, §18.5). There is NO `memories.*` RPC — the wire surface is
-- NOT PORTED (cancelled, not deferred): the original is a vestigial non-persisted in-memory Map
-- stub with no live consumers. Rows are written/read INTERNALLY and exposed to agents through the
-- agent→BE MCP callback (§6.8) as a context source. Upgrades src/features/memories/main/memories.service.ts (originally a non-persisted in-memory Map stub) to a durable table.
...
| `search.codebase { workspaceId, query }` | `codebase:search` **never-implemented stub returning `[]`, with no caller** — `src/features/notes/main/notes-primitives.ipc.ts` | **ripgrep/symbol-backed** (same engine as `inFiles` + a symbol pass) — this **exceeds** the original dead placeholder. Verified (2026-06-24) against canonical `augmentcode/intent` @ `6d97184` + auggie 0.29.0: auggie exposes **no structured codebase-retrieval CLI**, so `AuggieContextEngine::retrieve()` returns `Unavailable` instantly (no spawn) and ripgrep is the backing — **not a gap**. The `ContextEngine` trait (§8) + discovery + `doctor` availability are retained as forward-looking infra. |
...
**`export` action — NOT PORTING.** The original `accept-changes` service exposes an `export` action (write the change set to an `exportPath` on disk), but it has **no UI consumer** in `augmentcode/intent` and **will not exist** in the new Svelte/Tauri FE. `accept-changes.execute` therefore **permanently rejects `action:"export"`** — it is **cancelled, not deferred** (closing the earlier Milestone 8 deferral framed around `exportPath`/path-safety). All other execute actions ship (§17.2 table; Milestone 8 added the five history-rewriting ones).
...
### 18.5 `memories.*` — NOT PORTED (vestigial stub; internal context source only)

`src/features/memories/main/memories.service.ts` provides long-term agent memory (`list`/`get`/`create`/`update`/`delete`/`search`), but in `augmentcode/intent` it is a **vestigial, non-persisted in-memory `Map` stub** with **no renderer caller** — an internal/agent-MCP context source, not a user-facing feature. Accordingly there is **no `memories.*` client RPC**, and it is **NOT PORTED** (cancelled, not deferred). The `memories` table (§9.2/§9.12) is still created and `search.memories` (§5.15) scans it, so memories are written/read internally and surfaced to agents through the agent→BE MCP callback (§6.8); the data model is durable. A `memories.*` namespace could be added additively later **only if** a memories UI ever ships — but it is not on the porting roadmap.
