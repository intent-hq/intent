...
**Types (fixed catalog order):** `direct` (in-repo, no isolation — **always enabled**: the universal fallback every creation flow can use; a `sandbox.profiles.update` or raw `settings.update` that sets `direct.enabled = false` fails validation (§5.12), so its `enabled` reads `true` on every surface), `worktree` (linked git worktree — only offerable for local repository copies: `workspace.create` rejects it for `githubUrl`/`isNewRepo` flows, §5.1 flow-aware validation), `cow` (standalone CoW clone + per-agent CoW sandboxes, §5.5a), `microvm` (libkrun Linux guests running agents against their own CoW clone via virtio-fs — requires CoW; v1 is macOS Apple Silicon, local-only).
