...
```json
{
  ...
  "registeredCommitSha": "abc123…",     // optional; the root's HEAD commit SHA when FIRST registered (agent or auto-detect) — immutable once set (re-registration/merges never overwrite it); omitted when unknown (HEAD unreadable at registration, or a row predating the field awaiting the sweep's best-effort backfill)
