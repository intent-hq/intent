...
| script.stop | workspaceId (req), scriptId (req) | { ok, scriptId } — on a **non-running** script that carries the was-running marker this is the **dismiss** affordance: it durably clears `previouslyRunning`, emits a `script:state` snapshot (§6.5), and returns ok instead of erroring |
...
> - **Dismiss:** `script.stop` on a non-running script that carries the marker durably clears
>   it and returns ok (instead of erroring), and **emits a `script:state` event** carrying the
