# Intent Backend — JSON-RPC Protocol v7.1
...
| sandbox | 6 | cow.discard, cow.merge (§5.5a), image.check, options, profiles.list, profiles.update — the `sandbox.profiles.*` / `sandbox.options` / `sandbox.image.check` execution-environment profile surface is daemon-global (no `workspaceId`; §5.5b; v7.1) |
...
**Execution-environment selection (`workspace.create`, v7.1).** The optional
...
`executionEnvironment` (rows created before v7.1 introduced the field) keep the
...
**`executionEnvironment` (v7.1).** `"direct" | "worktree" | "cow" | "microvm"` (lowercase
...
`checkoutMode`; omitted for pre-existing rows created before v7.1 and for
...
### 5.5b `sandbox.profiles.*` / `sandbox.options` (execution-environment profiles) *(v7.1)*
...
- **`server` block.** The result advertises daemon capabilities so a client can gate UI right
  ...
  (daemon version string), `protocolVersion` (the JSON-RPC surface version, `"7.1"`), and
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-7f3a","protocolVersion":"7.1",
  "server":{ "locality":"remote","hasDisplay":false,"osArch":"linux/x86_64","version":"0.1.0",
    "protocolVersion":"7.1","capabilities":{ "liveState":true } } } }
```
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-9b21","protocolVersion":"7.1",
