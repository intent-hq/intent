# Intent Backend — JSON-RPC Protocol v7.2
...
The API exposes **336 dispatchable method names** across the following categories:

- **Router methods:** 297 methods dispatched via the main router (`router::dispatch`)
...
- **Client-served reverse RPCs:** 4 methods total — 2 are **dual-role** and counted within the 336 dispatchable names (`browser.exec`, `host.openInEditor`), and 2 are **daemon→client-only** reverse RPCs not in the dispatchable catalog (`host.openExternal`, `host.pickApplication`) — see §5.9 and §5.14

**Total:** 336 dispatchable names + 1 notification. Of the 4 reverse-RPC names, 2 (`browser.exec`, `host.openInEditor`) are dual-role — dispatchable client→server methods that are also issued daemon→client as reverse RPCs on remote connections — and 2 (`host.openExternal`, `host.pickApplication`) are daemon→client-only reverse RPCs, never dispatched client→server.
...
### Router methods by namespace (297 total)
...
| sandbox | 6 | cow.discard, cow.merge (§5.5a), image.check, options, profiles.list, profiles.update — the `sandbox.profiles.*` / `sandbox.options` / `sandbox.image.check` execution-environment profile surface is daemon-global (no `workspaceId`; §5.5b; v7.2) |
...
**Execution-environment selection (`workspace.create`, v7.2).** The optional
...
`executionEnvironment` (rows created before v7.2 introduced the field) keep the
...
**`executionEnvironment` (v7.2).** `"direct" | "worktree" | "cow" | "microvm"` (lowercase
...
`checkoutMode`; omitted for pre-existing rows created before v7.2 and for
...
### 5.5b `sandbox.profiles.*` / `sandbox.options` (execution-environment profiles) *(v7.2)*
...
- **`server` block.** The result advertises daemon capabilities so a client can gate UI right
  ...
  (daemon version string), `protocolVersion` (the JSON-RPC surface version, `"7.2"`), and
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-7f3a","protocolVersion":"7.2",
  "server":{ "locality":"remote","hasDisplay":false,"osArch":"linux/x86_64","version":"0.1.0",
    "protocolVersion":"7.2","capabilities":{ "liveState":true } } } }
```
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-9b21","protocolVersion":"7.2",
