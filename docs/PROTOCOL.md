...
| task.setRelations *(v6.8)* | noteId (req), dependsOn?: [taskNoteId], conflictsWith?: [taskNoteId] | { ok, noteId, dependsOn, conflictsWith } — writes the first-class task relations ([intent-hq/intentd#1100](https://github.com/intent-hq/intentd/pull/1100), monorepo#1974): `dependsOn` (hard ordering edges) / `conflictsWith` (advisory) on `TaskMetadata`, replace semantics per list (omitted param → kept, `[]` → cleared), echoed normalized (deduped, first-seen order); emits `note:updated`. Validation (rejected with `-32603`, detail in `error.data`): every id must name a **task** note in the same workspace, self-edges rejected, and a `dependsOn` write that would close a dependency cycle is rejected with the cycle path named (e.g. `dependsOn would create a cycle: b -> c -> b`); `conflictsWith` is advisory — no cycle check. The relations plus the computed `unmetDependsOn` (dep ids whose task note is not `complete`; missing and cancelled deps count as unmet) are projected — presence-detected, omitted when empty — on `task.getMyTask` (`taskMetadata` + top-level `unmetDependsOn`), `task.list` / `task.get` `WorkspaceTask` rows, and `note.listTasks` rows with a linked task note. `task.markAsTask` accepts the same optional `dependsOn?` / `conflictsWith?` params under identical validation (omitted params leave an existing task's relations untouched); `task.createPrerequisite` is unchanged for back-compat |
