...
- **Client-served reverse RPCs:** 4 methods total — 2 are **dual-role** and counted within the 308 dispatchable names (`browser.exec`, `host.openInEditor`), and 2 are **daemon→client-only** reverse RPCs not in the dispatchable catalog (`host.openExternal`, `host.pickApplication`) — see §5.9 and §5.14

**Total:** 308 dispatchable names + 1 notification. Of the 4 reverse-RPC names, 2 (`browser.exec`, `host.openInEditor`) are dual-role — dispatchable client→server methods that are also issued daemon→client as reverse RPCs on remote connections — and 2 (`host.openExternal`, `host.pickApplication`) are daemon→client-only reverse RPCs, never dispatched client→server.
...
| github.branches.listCached | owner (req), repo (req) | { cached: boolean, branches: string[], defaultBranch? } — **read-only, no network I/O**: branch names from the daemon's local repo cache (`.repo-cache/{owner}/{repo}`) — remote-tracking names (`refs/remotes/origin/*`, the `HEAD` symref excluded), sorted; `defaultBranch` derives from the `origin/HEAD` symref recorded at clone time and is **omitted when unresolvable**. A cold cache or foreign-origin repo → `{ cached: false, branches: [] }` — graceful, **never an error** (an explicit exception to the namespace's error conventions above, like `github.repoConfig.get`); invalid `owner`/`repo` path segments → `-32602`. FE consumption is cached-first: the branch picker renders this result instantly, then falls back to `github.branches.list` (and the repo's `defaultBranch`) when uncached (v6.2) |
