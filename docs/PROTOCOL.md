...
- **WSS:** the limit is enforced on both the WebSocket frame size and the total message size, and the connection is closed on violation. The daemon attempts to send a close frame with code **1009 (Message Too Big)** before terminating. A single over-limit frame fails fast on the frame header (its payload is not buffered), though the mid-write TCP reset may prevent the client from observing the close frame; a fragmented message is rejected once its accumulated fragments exceed the cap (so up to the limit may be buffered before rejection), and in that case the 1009 close frame is reliably delivered.
