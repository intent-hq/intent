# Intent Backend — JSON-RPC Protocol v4.1
...
Version 4.1 is an **additive** minor bump over 4.0: it adds the daemon-global `agent.listActive` router method (§5.5; monorepo#1395), which returns only agents with a turn currently in flight and avoids a `workspace.list` → `agent.list` fan-out.

The protocol version is advertised in two places:

- `client.hello` response: `{ protocolVersion: "4.1", server: { protocolVersion: "4.1", ... }, ... }` — the top-level `protocolVersion` is an explicit copy of `server.protocolVersion` so clients can version-check without digging into the `server` block (§5.17).
- `system.status` response: `{ protocolVersion: "4.1", ... }`
...
The API exposes **313 dispatchable method names** across the following categories:
...
- **Client-served reverse RPCs:** 4 methods (dual-role, counted within the 313 dispatchable names: `browser.exec`, `host.openExternal`, `host.openInEditor`, `host.pickApplication` — see §5.9 and §5.14)

**Total:** 313 dispatchable names + 1 notification. The 4 reverse-RPC names are dual-role: they are dispatchable client→server methods AND are also issued daemon→client as reverse RPCs on remote connections.
...
| agent.listActive *(v4.1)* | — (no workspaceId) | `{ streams: [{ agentId, sessionId, workspaceId, startTime }] }` — daemon-global agents whose runtime turn is currently in flight (`AgentManager::is_busy`, the same signal exposed as `AgentLite.isResponding`). `agentId` and `sessionId` are the same agent id; `workspaceId` is the agent's owning workspace; `startTime` is the session's `updatedAt` parsed as whole epoch milliseconds (`0` when unparsable). Idle agents are excluded. The implementation snapshots the in-memory busy set and reads only `updated_at` for those few agents; it never scans workspaces or calls `agent.list`. |
...
- **`server` block.** The result advertises daemon capabilities so a client can gate UI right
  ...
  (daemon version string), `protocolVersion` (the JSON-RPC surface version, `"4.1"`), and
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-7f3a","protocolVersion":"4.1",
  "server":{ "locality":"remote","hasDisplay":false,"osArch":"linux/x86_64","version":"0.1.0",
    "protocolVersion":"4.1","capabilities":{ "liveState":true } } } }
```
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-9b21","protocolVersion":"4.1",
