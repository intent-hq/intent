...
(`note.add`, `note.edit`, `note.editLines`, `note.setContent`, and the content arm of
`note.update`) auto-converts `@@@task` blocks in the resulting content into linked task
notes, and the write's result carries the conversion outcome: `convertedCount`,
`createdTaskNoteIds`, plus (v6.11, intentd#1133) `createdTasks` — `[{ key?, title,
noteId }]` in block order, parallel to `createdTaskNoteIds` — and `warnings`, both
always present (empty arrays when nothing applies). The fence line accepts the optional
