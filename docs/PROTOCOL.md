...
packaged sidecar's read-only cwd. The persisted `repositoryPath` is always the expanded
form. Expansion is `/`-separated only, and when the daemon cannot resolve a home
directory from its environment the path passes through unchanged (the clone then fails
as before). Progress streams as `git:clone:progress` frames and terminates in exactly
...
| git.clone | url (req), parentDir (req), targetName?, requestId? | { requestId, targetPath } — **streaming**: returns the ack promptly and pushes `git:clone:progress` frames followed by a terminal `git:clone:done` (§6.5). A leading `~` / `~/` in `parentDir` expands to the daemon user's home directory (`~user` forms pass through unchanged); the ack's `targetPath` carries the expanded path. `targetName` defaults to the URL basename (with `.git` stripped); rejected if it contains a path separator or would escape `parentDir`. `-32602` on missing/invalid params; `-32603` when the target path already exists or the event bus is not wired. |
