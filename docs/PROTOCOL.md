...
start/stop: the 0→1 running transition recomputes-and-emits immediately, and the running→idle
flip emits after the same debounce grace window as `workspace:activity-changed`, so the two
stay in lockstep. The emit-path enrichment (both the enriched list/get path and the lite
...
| agent (streaming) | agent:stream:start, agent:stream:activity, agent:stream:end | see §7 — `agent:stream:activity` is the **content-free rename** of `agent:stream:chunk` ([intent-hq/intentd#775](https://github.com/intent-hq/intentd/pull/775)); `agent:stream:start` is emitted **only** for agent-initiated (harness-wake) turns (§6.6) |
| chat (internal content feed, new in intentd) | chat:stream:delta | The content-bearing per-chunk stream payload ([intent-hq/intentd#775](https://github.com/intent-hq/intentd/pull/775) — where the former `agent:stream:chunk` content moved). data = { agentId, content, messageId, blockIndex, blockId, blockType, streamId? } (the §7.1 block-identity enrichment). Deliberately **outside** the `agent:*` family so `agent:*` / `agent:stream:*` `events.subscribe` filters never receive the high-volume transcript firehose — external subscribers get the content-free `agent:stream:activity` signal instead; the §7.1 `chat.subscribe` forwarder is its one consumer. **Transient / broadcast-only** (§7): never persisted, invisible to `event.query` (§5.10) |
...
**`agent:stream:chunk` → content-free `agent:stream:activity` ([intent-hq/intentd#775](https://github.com/intent-hq/intentd/pull/775)).**
The externally-broadcast per-chunk event is renamed `agent:stream:activity` and **no longer
carries transcript content**: its payload is `{ agentId, messageId }` only — a pure liveness
signal (busy tick / stall timestamps / watched-agent refresh hints). The content-bearing
per-chunk payload moved to the **internal** `chat:stream:delta` event (§6.5), deliberately
outside the `agent:*` family so `events.subscribe` firehose subscribers (`agent:*`,
`agent:stream:*`) never receive transcript content — the §7.1 `chat.subscribe` forwarder tails
it unchanged, so the snapshot+delta flow is unaffected. `agent:stream:activity` is
...
Both events remain **transient** (broadcast-only, never persisted). Version skew is benign:
an old client listening for `agent:stream:chunk` against a new daemon (or vice versa) loses
only liveness UX — transcripts are unaffected because `chat.subscribe` is unchanged.
...
| text token(s) — liveness signal | agent:stream:activity | { agentId, messageId } — **content-free** liveness tick ([intent-hq/intentd#775](https://github.com/intent-hq/intentd/pull/775); renamed from `agent:stream:chunk`), leading-edge throttled per agent (first chunk of a turn immediate, then ≤1/s; see the rename block above). The incremental text itself flows on the internal `chat:stream:delta` event — `{ agentId, content, messageId, blockIndex, blockId, blockType, streamId? }`, enriched with the §7.1 block-identity fields — which is outside the `agent:*` filter family and consumed by the §7.1 `chat.subscribe` forwarder |
