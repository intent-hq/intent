...
- `client.hello` response: `{ protocolVersion: "6.18", server: { protocolVersion: "6.18", ... }, ... }` — the top-level `protocolVersion` is an explicit copy of `server.protocolVersion` so clients can version-check without digging into the `server` block (§5.17).
- `system.status` response: `{ protocolVersion: "6.18", ... }`
