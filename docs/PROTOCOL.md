# Intent Backend — JSON-RPC Protocol v6.13

**Protocol Version:** `6.13`
...
The API exposes **326 dispatchable method names** across the following categories:

- **Router methods:** 287 methods dispatched via the main router (`router::dispatch`)
...
- **Client-served reverse RPCs:** 4 methods total — 2 are **dual-role** and counted within the 326 dispatchable names (`browser.exec`, `host.openInEditor`), and 2 are **daemon→client-only** reverse RPCs not in the dispatchable catalog (`host.openExternal`, `host.pickApplication`) — see §5.9 and §5.14

**Total:** 326 dispatchable names + 1 notification. Of the 4 reverse-RPC names, 2 (`browser.exec`, `host.openInEditor`) are dual-role — dispatchable client→server methods that are also issued daemon→client as reverse RPCs on remote connections — and 2 (`host.openExternal`, `host.pickApplication`) are daemon→client-only reverse RPCs, never dispatched client→server.
...
### Router methods by namespace (287 total)
...
| file | 11 | delete, exists, getAttachmentInfo, list, mkdir, placeAttachment, read, rename, stat, tree, write |
...
- **`server` block.** The result advertises daemon capabilities so a client can gate UI right
  ...
  (daemon version string), `protocolVersion` (the JSON-RPC surface version, `"6.13"`), and
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-7f3a","protocolVersion":"6.13",
  "server":{ "locality":"remote","hasDisplay":false,"osArch":"linux/x86_64","version":"0.1.0",
    "protocolVersion":"6.13","capabilities":{ "liveState":true } } } }
```
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-9b21","protocolVersion":"6.13",
