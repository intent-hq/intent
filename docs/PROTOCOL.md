...
| -32011 | Server overloaded | The daemon-wide cap on outstanding slow-path RPCs (`server.maxOutstandingRpcs`, default 256, `0` = unlimited) is reached, so the request is rejected immediately rather than queued. The cap is shared across every connection and both transports, and covers only dispatched slow-path work (`host.*`, `browser.*`, and router-dispatched methods); inline fast-path methods (subscriptions, `system.*`, `client.hello`) are never rejected. `message` is the fixed string `"Server overloaded"` with no `error.data`. Notification-shaped frames (no `id`) are dropped without a response. Clients should back off and retry. Changing the cap takes effect on daemon restart. |
