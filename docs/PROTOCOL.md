...
- **Plain `agent.stop`** (the keep-alive interrupt path — an ACP session exists to cancel; the hard-kill fallback taken when no `acpSessionId` exists does not flush) persists a synthetic **empty** interrupted assistant row (`contentBlocks: []`, `metadata.interrupted: true`, `metadata.stopReason: "interrupted"`) so the transcript durably records the stop, and the terminal `agent:stream:end` carries that row's `messageId`.
