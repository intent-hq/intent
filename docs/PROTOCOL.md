...
| workspace.setAutoCommit | workspaceId (req), enabled (req): boolean | { autoCommit: { enabled, source: "workspace" } } — persists the per-workspace override (survives daemon restart) and emits `workspace:updated` with `changes: { autoCommitEnabled }` (§6.5). -32602 on missing workspace, missing/non-boolean `enabled`, or the virtual Chief workspace. |
