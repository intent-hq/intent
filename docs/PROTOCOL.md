...
resolved as: per-call `language` → the `voice.language` setting (§5.12; trimmed, a
blank stored value is treated as unset) → none (provider auto-detection). The setting
is an optional ISO-639-1 string with no default, TOML-backed under `[voice]` like
`voice.provider`.
