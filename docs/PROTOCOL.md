...
| agent.getQueue | agentId (req) | { success, queue: QueuedMessage[] } — QueuedMessage = { id, content, queuedAt, position, turnId?, imageBlocks?, fileBlocks?, messageMetadata? } (shape as `agent.queueMessage`, including the monorepo#1022 `turnId?` correlation id) |
...
(§6.5/§7). The id is **stable across terminal-failure requeues** — the requeued queue entry gets
