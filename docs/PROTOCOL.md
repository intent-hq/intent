...
1. **Session persistence** — the pending request is persisted on the caller's session as
   ...
   **user-origin** delivery — `agent.sendMessage` (the FE/router front door),
   ...
   (skipped silently when none is pending).
...
carries raw transcript content**: its payload is `{ agentId, messageId, lastAgentResponse?,
...
text blocks, and `digest` the parsed digest span, both derived by the same extraction as the
§5.5 `AgentLite` live-turn preview overlay ([intent-hq/intentd#786](https://github.com/intent-hq/intentd/pull/786));
each field is **omitted** (never an empty string) until derivable, so an early turn that has
streamed no text / no digest carries neither and clients keep their previous preview. Mid-turn
a chunk boundary can split a digest marker, letting partial tag text surface for up to one
throttle window; the next derivation strips the completed tag. The content-bearing
