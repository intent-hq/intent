...
the queue head with `interruptPriority: true`, ahead of every parked entry, so the model learns
of the dismissal before the released backlog drains — see the `agent.dismissQuestions` row for
the wording, `questions_dismissed` metadata, idempotency, and fail-soft contract) — or (3) a
...
//    dismissal notice delivered to the model (immediate turn when idle, else queue-head with
//    interruptPriority — intentd#892), carrying messageMetadata:
