# Intent Backend — JSON-RPC Protocol v6.15

**Protocol Version:** `6.15`
...
- `client.hello` response: `{ protocolVersion: "6.15", server: { protocolVersion: "6.15", ... }, ... }` — the top-level `protocolVersion` is an explicit copy of `server.protocolVersion` so clients can version-check without digging into the `server` block (§5.17).
- `system.status` response: `{ protocolVersion: "6.15", ... }`
...
The API exposes **327 dispatchable method names** across the following categories:

- **Router methods:** 288 methods dispatched via the main router (`router::dispatch`)
