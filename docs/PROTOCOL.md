# Intent Backend — JSON-RPC Protocol v6.6
...
- **Router methods:** 277 methods dispatched via the main router (`router::dispatch`)
...
- **Client-served reverse RPCs:** 4 methods total — 2 are **dual-role** and counted within the 316 dispatchable names (`browser.exec`, `host.openInEditor`), and 2 are **daemon→client-only** reverse RPCs not in the dispatchable catalog (`host.openExternal`, `host.pickApplication`) — see §5.9 and §5.14

**Total:** 316 dispatchable names + 1 notification. Of the 4 reverse-RPC names, 2 (`browser.exec`, `host.openInEditor`) are dual-role — dispatchable client→server methods that are also issued daemon→client as reverse RPCs on remote connections — and 2 (`host.openExternal`, `host.pickApplication`) are daemon→client-only reverse RPCs, never dispatched client→server.
...
### Router methods by namespace (277 total)
...
| file | 10 | delete, exists, list, mkdir, placeAttachment (§5.9; v6.5), read, rename, stat, tree, write |
...
| sandbox | 6 | cow.discard, cow.merge (§5.5a), image.check, options, profiles.list, profiles.update — the `sandbox.profiles.*` / `sandbox.options` / `sandbox.image.check` execution-environment profile surface is daemon-global (no `workspaceId`; §5.5b; v6.6) |
...
| workspace.unarchive | workspaceId (req) | { workspace: Workspace } — mirror of `workspace.archive`; returns the refreshed record with `archived: false` / `status: "Active"` and `archivedAt` cleared. Emits `workspace:updated` with `changes: { archived: false, status: "Active", archivedAt: null }` — an explicit JSON `null` so clients clear the field (§6.5). Re-kicks the queue drains parked by the archived gates so parked messages deliver without a manual kick; cancelled hooks and PR monitors are NOT resurrected (see the archive active-work teardown block below). -32602 if not found. |
...
**Execution-environment selection (`workspace.create`, v6.6).** The optional
...
`executionEnvironment` (rows created before v6.6 introduced the field) keep the
...
- **Active PR monitors are cancelled** *([intent-hq/intentd#1067](https://github.com/intent-hq/intentd/pull/1067); monorepo#1828)*.
  ...
  `workspace:displayStatus-changed` recompute (§6.5) — the demotion lands with the
  sweep's last cancel — so an archived workspace's `displayStatus` no longer reads
  `in_progress` indefinitely off a stale active-monitor signal (§5.1 step 3).
...
**`executionEnvironment` (v6.6).** `"direct" | "worktree" | "cow" | "microvm"` (lowercase
...
`checkoutMode`; omitted for pre-existing rows created before v6.6 and for
...
guarded no-op when the flag is not `none`, and skipped entirely when the workspace is
`Archived` at drain time — fail-open on a workspace-read error;
[intent-hq/intentd#1075](https://github.com/intent-hq/intentd/pull/1075)), `workspace.dismissAttention`,
...
### 5.5b `sandbox.profiles.*` / `sandbox.options` (execution-environment profiles) *(v6.6)*
...
| file.placeAttachment | fileName (req), data? (base64) / sourcePath? (exactly one, validated in the service → -32602) | { ok, path, fileName, size } — *(v6.5)* daemon-mediated attachment placement into the git-ignored `.intent/attachments/` workspace directory ([monorepo#1948](https://github.com/intent-hq/monorepo/issues/1948)): the client-supplied name is reduced to a safe basename, collisions get a numeric suffix (`<stem>-2<ext>`, … then a UUID fallback), and the name is claimed atomically (`create_new`) so concurrent placements never overwrite each other; `path` is workspace-relative, `size` the byte length written |
...
- **`server` block.** The result advertises daemon capabilities so a client can gate UI right
  ...
  (daemon version string), `protocolVersion` (the JSON-RPC surface version, `"6.6"`), and
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-7f3a","protocolVersion":"6.6",
  "server":{ "locality":"remote","hasDisplay":false,"osArch":"linux/x86_64","version":"0.1.0",
    "protocolVersion":"6.6","capabilities":{ "liveState":true } } } }
```
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-9b21","protocolVersion":"6.6",
