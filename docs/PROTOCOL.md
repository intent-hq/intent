# Intent Backend — JSON-RPC Protocol v5.2
...
- **Delegation reasoning-effort resolution (additive)** — `agent.delegate` and
  ...
  `reasoningEffort` frontmatter scalar; (4) unset. The resolved level is then **validated
  ...
  bare-model ownership guard's "absence of evidence is not a mismatch" rule (§5.5).
  `agent.create` applies the **same validation** to its `reasoningEffort` param against its
  own resolved model (§5.5 `agent.create`); it has no specialist fallback rungs — the param
  is the only source, so only steps (1) and the validation apply.
...
- **`server` block.** The result advertises daemon capabilities so a client can gate UI right
  ...
  (daemon version string), `protocolVersion` (the JSON-RPC surface version, `"5.2"`), and
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-7f3a","protocolVersion":"5.2",
  "server":{ "locality":"remote","hasDisplay":false,"osArch":"linux/x86_64","version":"0.1.0",
    "protocolVersion":"5.2","capabilities":{ "liveState":true } } } }
```
...
{ "jsonrpc":"2.0","id":1,"result":{ "clientId":"cli-9b21","protocolVersion":"5.2",
...
10000, ttlMs? }` — `name` is a short human-readable description shown to the user
...
**Cancel is ownership-scoped on the MCP side** (within v5.1, behavior only;
