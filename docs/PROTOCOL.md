...
3. **`agent:attention-requested` event** — the self-sufficient (§6.7) toast-driving event,
   ...
   **omitted entirely otherwise — never `null`**; the FE suppresses its sticky attention
   toast when the field is present (the parent wake in step 5 is the delegated child's
   attention surface, not a user-facing toast).
