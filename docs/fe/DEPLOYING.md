...
2. **Beta-first guard**: verifies the version was promoted to beta — the current beta channel version (read from the `beta` release's `latest-mac.yml` feed) must be >= the promoted version, the invariant being that beta can never be behind stable. Fails fast before any channel asset is downloaded or uploaded when the beta feed is missing or unparseable (the version was never promoted to beta) or the beta version is behind the promoted one. Dispatching with `skip_beta_check: true` bypasses the guard for emergency promotions, logged as a warning
